...
var heuristic = function(state, maximizingPlayer){
	...
	//An example.
		var lastMove = state.lastMove || 0;
    var linesOfLengthTwoForX = state.numLines(2, 'x')
    var linesOfLengthThreeForX = state.numLines(3, 'x')
		const middleBonus = (3 - Math.abs(lastMove - 3)) * 10;
	//An example.
    var linesOfLengthTwoForOh = state.numLines(2, 'o')
    var linesOfLengthThreeForOh = state.numLines(3, 'o')

		function heuristicHelper(player) {
			let scoreX = 0;
			let scoreOh = 0;
			let scoreFinal = 0;
			if (state.numLines(4, player)) return 10000000;
			if (state.numLines(4, minimizingPlayer)) return -10000000;
			if (player === 'x') {
				scoreX += linesOfLengthTwoForX * 2;
				scoreX += linesOfLengthThreeForX * 5;
				scoreX += middleBonus;

				// scoreOh -= middleBonus;
				scoreOh -= linesOfLengthTwoForOh * 2;
				scoreOh -= linesOfLengthThreeForOh * 5;
				scoreFinal = (scoreX * 100 / 21) + (scoreOh * 100 / 21);
			} else {
				scoreOh += linesOfLengthTwoForOh * 2;
				scoreOh += linesOfLengthThreeForOh * 5;
				scoreOh += middleBonus;
	...
	return heuristicHelper(maximizingPlayer);
}
...
var minimax = function(state, depth, maximizingPlayer){
	...
	//Your code here.
	if (possibleStates.length === 0 || !depth) {
		return heuristic(state, maximizingPlayer);
	}
	if (currentPlayer === maximizingPlayer) {
		return possibleStates.map(nextState => {
